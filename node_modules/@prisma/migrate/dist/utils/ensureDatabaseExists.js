...
async function getDbInfo(schemaPath) {
  ...
  const config = await (0, import_sdk.getConfig)({ datamodel });
  ...
  try {
    const credentials = (0, import_sdk.uriToCredentials)(url);
...
async function ensureCanConnectToDatabase(schemaPath) {
  ...
  const canConnect = await (0, import_sdk.canConnectToDatabase)(activeDatasource.url.value, schemaDir);
  if (canConnect === true) {
    return true;
  } else {
...
async function ensureDatabaseExists(action, forceCreate = false, schemaPath) {
  ...
  if (forceCreate) {
    if (await (0, import_sdk.createDatabase)(activeDatasource.url.value, schemaDir)) {
      if (activeDatasource.provider === "sqlserver") {
        return `SQL Server database created.
`;
      }
      const credentials = (0, import_sdk.uriToCredentials)(activeDatasource.url.value);
...
`;
  ...
  if (response.value) {
    await (0, import_sdk.createDatabase)(connectionString, schemaDir);
  } else {
    process.exit(0);
  }
}
